...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _insertAlertPayload(Map<String, dynamic> payload) async {
    ...
    for (var attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        final missingColumns = _extractMissingColumns(errorMessage);
        if (missingColumns.isEmpty && errorMessage.contains('schema cache')) {
          missingColumns.addAll(['latitude', 'longitude', 'location_address']);
        }
  ...
  bool _isMissingSchemaColumnError(Object e) {
    ...
    return message.contains("Could not find the '") ||
        ...
        message.contains('does not exist'); // PostgreSQL: "column alerts.status does not exist"
  }
  ...
  List<String> _extractMissingColumns(String message) {
    final patterns = [
      ...
      // Extract just the column part (after the dot if table-qualified)
